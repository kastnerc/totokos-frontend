...
export default {
  setup() {
    ...
    const handleSubmit = async () => {
      ...
      if (!categoryRegex.test(product.value.id_category)) {
        alert("Category ID must be numeric");
        return;
      }
      if (!nameRegex.test(product.value.product_name)) {
        alert("Product name must only contain letters and spaces");
        return;
      }
      if (!priceRegex.test(product.value.product_price)) {
        alert("Price must be a valid number, optionally with up to two decimal places");
        return;
      }
      if (!descriptionRegex.test(product.value.description)) {
        alert("Description cannot be empty");
        return;
      }
      if (!stockRegex.test(product.value.stock)) {
        alert("Stock must be numeric");
        return;
      }
      if (!dateRegex.test(product.value.expiry_date)) {
